...
import { tournamentApi, Match } from "@/api/tournamentApi";
...
export default function FixtureGeneration() {
  ...
  const loadBracket = async (id: string) => {
    ...
    setIsLoading(true);
    try {
      const data = await tournamentApi.getMatchesByTournament(id);
      setMatches(data);
      setShowBracket(true);
      setFormat("knockout");
  ...
  const handleUpdateScore = async (matchId: string, score1: number, score2: number) => {
    ...
    await tournamentApi.updateMatchScore(matchId, score1, score2);
  ...
  const handleGenerate = async () => {
    ...
    setShowBracket(true);
    setMatches([]); // Clear API matches when generating local fixtures
